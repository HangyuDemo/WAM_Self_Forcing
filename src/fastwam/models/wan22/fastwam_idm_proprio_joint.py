...
class FastWAMIDMProprioJoint(FastWAMIDM):
    ...
    def infer_joint(
        ...
        self.eval()
        if self.infer_proprio_scheduler is None or self.proprio_pred_dim is None:
            raise RuntimeError("Proprio-joint inference enabled but branch is not initialized.")

        if action is not None:
            logger.warning(
                "`FastWAMIDMProprioJoint.infer_joint` ignores `action` input; "
                "video/proprio are denoised in stage-1 first."
            )
        ...
        for step_t_video, step_delta_video in zip(infer_timesteps_video, infer_deltas_video):
            ...
            pred_proprio = self._predict_proprio_noise(
                ...
                timestep_proprio=timestep_video,
                context=context,
                context_mask=context_mask,
            )
        ...
        return {
            ...
            "proprio_latent": latents_proprio[0].detach().to(device="cpu", dtype=torch.float32),
            "proprio": self._expand_proprio_to_video_frames(
                latents_proprio.detach(), num_video_frames=int(num_video_frames)

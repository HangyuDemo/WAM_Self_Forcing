...
class FastWAMIDMJEPA(FastWAMIDM):
    ...
    def infer_action(self, *args, **kwargs):
        out = self.infer_joint(*args, **kwargs)
    ...
    def infer_joint(
        ...
        # Build cond-video tokens and predict JEPA trajectory from generated video tokens.
        timestep_video_cond = torch.zeros((1,), dtype=latents_video.dtype, device=self.device)
        video_pre_cond = self.video_expert.pre_dit(
            x=latents_video,
            timestep=timestep_video_cond,
            context=context,
        ...
        tokens_per_frame = int(video_pre_cond["meta"]["tokens_per_frame"])
        pred_jepa_full = self._predict_jepa_trajectory(
            pred_video_tokens=video_pre_cond["tokens"],
            tokens_per_frame=tokens_per_frame,
            target_steps=max(int(latent_t), 1),
        )
        jepa_cond_ctx = self.dit.jepa_action_ctx_proj(pred_jepa_full.to(dtype=context.dtype))
        jepa_cond_mask = torch.ones(
            (1, jepa_cond_ctx.shape[1]),
            dtype=torch.bool,
            device=context_mask.device,
        )
        action_context = torch.cat([context, jepa_cond_ctx], dim=1)
        ...
        # Stage 2: denoise action using JEPA-augmented context.
        ...
        decoded_video = self._decode_latents(latents_video, tiled=tiled)
        return {
            "video": decoded_video,
            "action": latents_action[0].detach().to(device="cpu", dtype=torch.float32),
            "jepa_trajectory": pred_jepa_full[0].detach().to(device="cpu", dtype=torch.float32),
        }

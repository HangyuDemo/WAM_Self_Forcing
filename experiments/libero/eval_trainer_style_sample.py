...
def _save_two_row_tensor_video(
    top_video_tensor: torch.Tensor,
    bottom_video_tensor: torch.Tensor,
    path: Path,
    fps: int,
) -> None:
    if top_video_tensor.shape != bottom_video_tensor.shape:
        raise ValueError(
            f"Two-row tensor video shape mismatch: top={tuple(top_video_tensor.shape)} "
            f"bottom={tuple(bottom_video_tensor.shape)}"
        )
    stitched_video_tensor = torch.cat([top_video_tensor, bottom_video_tensor], dim=2).contiguous()
    stitched_frames = []
    for t in range(stitched_video_tensor.shape[1]):
        frame = (stitched_video_tensor[:, t].permute(1, 2, 0).clamp(0.0, 1.0).numpy() * 255.0).astype(np.uint8)
        stitched_frames.append(Image.fromarray(frame))
...
def _infer_local_clip_for_rollout(model, dataset, sample_index: int, cfg: DictConfig) -> dict[str, Any]:
    raw_sample = dataset[sample_index]
    sample = Wan22Trainer._to_batched_eval_sample(raw_sample)

    prompt = sample["prompt"][0]
    video0 = sample["video"][0]
    # For local-dataset pred_rollout evaluation, we only use the local sample
    # to provide the initial frame / prompt / proprio. Action is generated by
    # the model itself, then executed in the simulator.
    action = None
    proprio = sample["proprio"][0, 0] if sample["proprio"] is not None else None
    input_image = video0[:, 0].unsqueeze(0)
    _, num_frames, _, _ = video0.shape

    eval_seed_cfg = cfg.EVALUATION.get("sample_seed")
    eval_seed = int(cfg.get("seed", 42) if eval_seed_cfg is None else eval_seed_cfg)
    ...
    pred_video = _select_predicted_future_frames(pred["video"], cfg)
    pred_action = pred.get("action", None)
    if pred_action is None:
        raise ValueError("Local pred_rollout mode requires model.infer(...) to return `action`.")

    denorm_action = _denormalize_action(pred_action, dataset.lerobot_dataset.processor)[0]
...
def _run_local_dataset_pred_rollout(
    model: torch.nn.Module,
    dataset,
    cfg: DictConfig,
    *,
    ...
    sample_index = _select_eval_index(dataset, cfg)
    clip = _infer_local_clip_for_rollout(model, dataset, sample_index, cfg)
    ...
    try:
        ...
        current_replan_idx = 0
        replan_count = 1
        pending_actions = np.asarray(clip["denorm_action"], dtype=np.float32)[:replan_steps].tolist()
        current_predicted_future_clip = {
            "replan_idx": current_replan_idx,
            "rollout_frames": [get_libero_image(obs)],
            "pred_frames": list(clip["pred_video"]),
        }
        current_replan_step = 0

        while total_action_steps < max_steps:
            if len(pending_actions) == 0:
                action_chunk, imgs, predicted_future_frames = _predict_action_chunk(
                    obs=obs,
                    task_description=task_description,
                ...
                if predicted_future_frames is None:
                    raise ValueError(
                        "Local-dataset pred_rollout mode requires predicted future video frames after simulator handoff. "
                        "Please keep EVALUATION.visualize_future_video=true."
                    )
                current_replan_idx += 1
                replan_count += 1
            ...
            if current_predicted_future_clip is not None:
                ...
                if done or len(pending_actions) == 0:
                    ...
                    if len(current_predicted_future_clip["pred_frames"]) != len(current_predicted_future_clip["rollout_frames"]):
                        raise ValueError(
                            "Local-init rollout/pred frame count mismatch: "
                            f"pred={len(current_predicted_future_clip['pred_frames'])} "
                            f"rollout={len(current_predicted_future_clip['rollout_frames'])}."
                        )
                    all_pred_frames.extend(current_predicted_future_clip["pred_frames"])
        ...
        if len(all_pred_frames) != len(all_rollout_frames):
            raise ValueError(
                "Local-init pred_rollout frame count mismatch after stitching: "
                f"pred={len(all_pred_frames)} rollout={len(all_rollout_frames)}."
            )

        sample_tag = f"sample_{clip['sample_index']:06d}"
        video_path = Path(
            save_prediction_video(
                str(output_dir),
                all_rollout_frames,
                all_pred_frames,
                f"{sample_tag}_task{task_id}_trial{trial_index}",
                "localdataset",
                success=bool(done),
                task_description=task_description,
                fps=int(cfg.EVALUATION.get("fps", 8)),
        ...
        return {
            ...
            "sample_indices": [int(clip["sample_index"])],
            "num_clips": 1,
            "sequence_sampling_mode": "single_clip_local_init_then_online",
            "sequence_stride": None,
            "pred_rollout_source": "local_dataset",
            "rollout_task_suite_name": str(cfg.EVALUATION.task_suite_name),
            "rollout_task_id": int(cfg.EVALUATION.task_id),
            "rollout_task_description": task_description,
            "rollout_done": bool(done),
            "env_trial_index": int(trial_index),
            "rollout_action_steps_executed": int(total_action_steps),
            "replan_count": int(replan_count),
            "num_pred_frames": int(len(all_pred_frames)),
            "num_rollout_frames": int(len(all_rollout_frames)),
            "dataset_prompt": clip["prompt"],
        }
    finally:
        env.close()
...
def _evaluate_clip(model, dataset, sample_index: int, cfg: DictConfig) -> dict[str, Any]:
    ...
    return {
        ...
        "vae_video_tensor": vae_video_tensor,
...
def main(cfg: DictConfig):
    ...
    if output_video_mode not in {"pred_vae_gt", "pred_rollout", "pred_gt"}:
        raise ValueError(
            f"Unsupported EVALUATION.output_video_mode={output_video_mode}. "
            "Expected one of: ['pred_vae_gt', 'pred_rollout', 'pred_gt']."
        )

    if output_video_mode == "pred_rollout":
        pred_rollout_source = str(cfg.EVALUATION.get("pred_rollout_source", "online")).strip().lower()
        if pred_rollout_source not in {"online", "local_dataset"}:
            raise ValueError(
                f"Unsupported EVALUATION.pred_rollout_source={pred_rollout_source}. "
                "Expected one of: ['online', 'local_dataset']."
            )
        with open_dict(cfg):
        ...
        metrics = {
            ...
            "pred_rollout_source": pred_rollout_source,
            "rollout_task_suite_name": str(cfg.EVALUATION.task_suite_name),
            "rollout_task_id": int(cfg.EVALUATION.task_id),
        }

        if pred_rollout_source == "online":
            ...
            metrics["dataset_stats_path"] = str(dataset_stats_path)
            metrics.update(rollout_metrics)
            metrics_path = output_dir / "localdataset--metrics.json"
    ...
    if output_video_mode == "pred_vae_gt":
        ...
        pred_video_tensor = torch.cat([item["pred_video_tensor"] for item in clip_results], dim=1).contiguous()
        gt_video_tensor = torch.cat([item["gt_video_tensor"] for item in clip_results], dim=1).contiguous()
        video_path = output_dir / f"{base_name}--pred-gt.mp4"
        _save_two_row_tensor_video(
            top_video_tensor=pred_video_tensor,
            bottom_video_tensor=gt_video_tensor,
            path=video_path,
            fps=int(cfg.EVALUATION.get("fps", 8)),
        )

    ...
    with output_file.open("w", encoding="utf-8") as f:
        for suite_name in task_suite_names:
            ...
            for task_id in range(n_tasks):
                ...
                total_tasks += 1
    print(f"Task list created: {output_file} (total={total_tasks})")
    return output_file
    ...
    env.update(
        {
            ...
            "EVAL_SINGLE_SCRIPT": "experiments/libero/eval_libero_single.py",
        }
    )

    print("Starting IDM evaluation manager...")
...
@hydra.main(version_base="1.3", config_path="../../configs", config_name="sim_libero_idm.yaml")
def main(cfg: DictConfig):
    ...
    if bool(manager.get("create_only", False)):
        print("create_only=True, only create task list and exit.")
        return
